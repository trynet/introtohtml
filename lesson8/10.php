<!DOCTYPE html>

<html>


<head>

<title>Lesson 8: Data Tables</title>

<meta charset="utf-8" />

<link rel="stylesheet" href="../styles/styles.css" />

</head>

<body>


<div id="wrapper">

<?php include("../includes/header.php"); ?>

<div class="clearfix">

<section id="main">


 <h1>Lesson 8: Data Tables</h1>


<p class="introduction">Gone are the days when tables were used to layout the design of your
          web pages. Sure, you'll still come across an old page (let us hope
          it's an old page) that uses tables for layout but there is no need
        to do that anymore.</p>
        
        <p>Like all things that deal with a page's appearance, we have CSS to
          do all the layout work we'll ever need in a web page.</p>
          
        <p>When you have the need to present information that requires rows and
          columns - trains schedules, calendars, math tables (math tables? -
          yuck) and such - then an HTML table is your way to go. </p>

<p id="learn">In this lesson we'll go over:</p>

	<ul id="highlights">
	<li>Rows And Columns</li>
<li>The th Tag</li>
<li>The caption Tag</li>
<li>Spanning Multiple Rows And Columns</li>
<li>thead, tbody And tfoot</li>
</ul>
<p id="cute_line">Setting Tables!!</p>


<a id="next" href="20.html">Time To Learn About Using Tables The Right Way</a>


</section>



<?php include("../includes/sidebar.php");
print_sidebar(8,1);
?>


</div>  <!-- ned clearfix -->

<?php include("../includes/footer.php"); ?>


</div> <!-- end wrapper -->

</body>

</html>




